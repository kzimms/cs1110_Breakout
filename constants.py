...
try:
   if (not sys.argv is None and len(sys.argv) == 3):
        ...
        if (bs_in_row > 0 and brick_rows > 0):
            ...
            BRICK_WIDTH    = GAME_WIDTH / BRICKS_IN_ROW - BRICK_SEP_H
            #: remainder of rows when dividing BRICK_ROWS by number of colors
            REMAIN = BRICK_ROWS%5
            #: number of rows with same color
            RC = BRICK_ROWS/5
            #:alters which rows are which color based on an inputed number
            # rows
            ROW_COLORS = (([colormodel.RED]*RC)+([colormodel.ORANGE]*RC)+
              ([colormodel.YELLOW]*RC)+([colormodel.GREEN]*RC)+
              ([colormodel.CYAN]*(RC + REMAIN)))
            #: minimizes the standard deviation of color distribution
            # over all rows
            if 5 - REMAIN < REMAIN:
               BR = 5 - REMAIN + BRICK_ROWS
               RC = BR/5
               ROW_COLORS = (([colormodel.RED]*RC)+([colormodel.ORANGE]*RC)+
                 ([colormodel.YELLOW]*RC)+([colormodel.GREEN]*RC)+
                 ([colormodel.CYAN]*(RC)))
...
#: the vertical velocity of ball when initialized
BALL_VY = -5.0
#: state when the game is over (won or lost)
STATE_COMPLETE = 4